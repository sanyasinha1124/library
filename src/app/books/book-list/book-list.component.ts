...
//     next: () => alert('Book borrowed successfully'),
...
export class BookListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  confirmBorrow(): void {
    ...
    this.issueService.borrowBook(this.selectedBook.id).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
        alert('✅ "' + this.selectedBook?.title + '" borrowed! Due in 14 days.');
        this.loadBooks();
      },
      error: (err: { error: { error: any; }; }) => { alert('❌ ' + (err.error?.error || 'Could not borrow book.')); }
    });
  }
